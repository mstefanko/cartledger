...
CREATE TABLE product_enrichment_jobs_new (
...
INSERT INTO product_enrichment_jobs_new (
...
ALTER TABLE product_enrichment_jobs_new RENAME TO product_enrichment_jobs;
...
CREATE TABLE product_identifiers_new (
    id                TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
    household_id      TEXT NOT NULL REFERENCES households(id) ON DELETE CASCADE,
    product_id        TEXT NOT NULL REFERENCES products(id) ON DELETE CASCADE,
    ...
    source            TEXT NOT NULL DEFAULT 'manual'
        CHECK (source IN ('manual', 'line_item', 'legacy_upc', 'enrichment', 'import', 'user_accept', 'receipt_review_scan')),
...
INSERT INTO product_identifiers_new (
...
ALTER TABLE product_identifiers_new RENAME TO product_identifiers;
...
CREATE TABLE line_item_identifier_observations_new (
    id                TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
    ...
    source            TEXT NOT NULL DEFAULT 'receipt'
        CHECK (source IN ('receipt', 'manual', 'import', 'repair', 'receipt_review_scan')),
    confidence        REAL CHECK (confidence IS NULL OR (confidence >= 0 AND confidence <= 1)),
    created_at        DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
);

INSERT INTO line_item_identifier_observations_new (
...
ALTER TABLE line_item_identifier_observations_new RENAME TO line_item_identifier_observations;
...
CREATE TABLE product_aliases_new (
    ...
    source           TEXT NOT NULL DEFAULT 'legacy'
        CHECK (source IN ('legacy', 'receipt_match', 'manual_match', 'user_alias', 'import', 'enrichment', 'receipt_review_scan')),
    confidence       REAL CHECK (confidence IS NULL OR (confidence >= 0 AND confidence <= 1)),
...
INSERT INTO product_aliases_new (
...
ALTER TABLE product_aliases_new RENAME TO product_aliases;
